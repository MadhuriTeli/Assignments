...
public class Student extends BaseEntity {
	...
	@ManyToOne(fetch = FetchType.LAZY) // many to one multiplicity
	@JoinColumn(name = "course_id") // specifying FK col name
	private Course courseEnrolled;
	@ManyToMany(mappedBy = "students")
	private Set<Project> projects = new HashSet<>();
	// one to one uni dir association between Student (entity) -----> AdharCard
	// (value)
	@Embedded // optional , added only for clarity
	private AdharCard card;// Composite Value Type : single
	// one to many uni dir asso between Student(Ent) ---> Edu Qual.(Composite val
	// type)
	...
	// one to many uni dir asso between Student(Ent) --->Hobbies(String : basic
	// value)
	@ElementCollection
