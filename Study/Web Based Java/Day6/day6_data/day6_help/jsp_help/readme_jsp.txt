...
2. Ease of development --- JSP pages are automatically translated by W.C in to servlet & compiled & deployed.

3. Can use web design tools -- for faster development (RAD --rapid application development) tools.

JSP API : part of J2EE specification 
...
2. JSP's implicit objects (available only to _jspService) -- available to scriptlets,expressions
...
2.6 pageContext  : current page environment : javax.servlet.jsp.PageContext(this class stores references to implicit objects viz -- exception,out,config,session)
...
eg : If you create test.jsp , Tomcat will translate it into a public class test_jsp : 
page => instance of this class.


3. Scripting elements : To include the java content within JSP : to make it dynamic.

3.1 Scriptlets : Can be used to add the java code directly . AVOID scriptlets . (Use only till you learn Javabeans & custom tags or JSTL,). we will use use the scriptlets to add : Req. processing logic,Business logic , Presentation logic)
...
param =name of the map containing request parameters
pageScope=name of the map containing page scoped attrs
requestScope=name of the map containing request scoped attrs
sessionScope=name of the map containing session scoped attrs
applicationScope=name of the map containing application(=context) scoped attrs
pageContext --- instance of PageContext's sub class
cookie --name of the map containing cookies(cookie objects)
initParam --name of the  map containing context params.
...
${sessionScope.user_details} ----sessionScope.get("user_details") ---> to string ---> sent to clnt
OR
session.getAttribute("user_Details") --->to string ---> sent to clnt

${param.user_nm} ---param.get("user_nm") --value --to string ---> clnt 
...
eg : Which will be faster for suppose : you have added a session scoped attribute
<%
...
3.3 JSP declarations (Represents  : private members of the translated servlet class)
syntax : <%! JSP declaration block %> (outside <body> tag within JSP)
Usage : 1. for creating page scoped java variables & methods (instance vars & methods/static members) : default scope => private
...
Via include directive ---- contents are included @ Translation time.--- indicates page scope(continuation of the same page i.e continuation of the _jspService).
...
2. Must have default constr.(MUST in JSP using JB scenario)
