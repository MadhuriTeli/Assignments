...
public class UserDepositController {
	...
	@PostMapping("/deposit")
	public String showDeposit(HttpSession hs,Model map,@RequestParam String startDate,@RequestParam String endDate) {
		List<Deposit> deposits = null;
		User u = null;
		try {
			u = (User) hs.getAttribute("user");
			deposits = service.getDeposit(Date.valueOf(startDate), Date.valueOf(endDate),u);
	...
	public String ProcessDepositForm(Model map,@RequestParam double balance,@RequestParam String maturity,@RequestParam String bankName,HttpSession hs) {
		...
		try {
			...
			map.addAttribute("status","Failed to add" + e);
		}
		return "/bank/dashboard";
	}
	...
	public String redirectAddress() {
		return "/bank/addAddress";
	}
	
	@PostMapping("/addAddress")
	public String ProcessAddressForm(Model map,@RequestParam String city,@RequestParam String state,HttpSession hs) {
		User u = null;
		try {
			u = (User) hs.getAttribute("user");
			map.addAttribute("status","Add Address successful");
			service.addAddress(u,new Address(city,state));
		}catch(RuntimeException e) {
			map.addAttribute("status","Failed to add" + e);
		}
		return "/bank/dashboard";
	...
	public String getAddressForm(Model map,HttpSession hs) {
		User u = null;
		try {
			u = (User) hs.getAttribute("user");
			map.addAttribute("address",service.getAddress(u));
		}catch(RuntimeException e) {
			map.addAttribute("status","Failed to get Address" + e);
		}
		return "/bank/dashboard";
	}
